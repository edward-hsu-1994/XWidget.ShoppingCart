    ...
    public class SecondPieceDiscount : ICoupon {
        ...
        public void Use(Order order) {
            foreach (var item in order.Items.Where(x => !(x is CouponOrderItem) && DiscountItems.Contains(x.Id)).ToArray()) {
                ...
                var canUseCount = item.Count - item.CouponUsed.Values.Sum(x => x);
                if (canUseCount < 2) {
                    continue;
                }

                if (Count.HasValue) {
                    coupon.Count = Math.Min(Count.Value, (uint)Math.Floor(((double)canUseCount) / 2));
                } else {
                    coupon.Count = (uint)Math.Floor(((double)canUseCount) / 2);
                }
                if (!item.CouponUsed.ContainsKey(Id)) {
                    item.CouponUsed[Id] = 0;
                }
                item.CouponUsed[Id] += coupon.Count * 2;
